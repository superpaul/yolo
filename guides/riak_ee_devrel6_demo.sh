...
for d in dev{1..6}; do sed -e 's/riak_kv_bitcask_backend/riak_kv_eleveldb_backend/' -i.bak $d/etc/app.config; done
grep storage_backend dev?/etc/app.config

## start nodes
for d in dev{1..6}; do $d/bin/riak start; done

## check nodes are up
for d in dev{1..6}; do $d/bin/riak ping; done
ps -ef | grep beam

## cover clustering, the ring, consistent hashing, partitions, vnodes, replicas etc
for d in dev{2..3}; do $d/bin/riak-admin cluster join dev1@127.0.0.1; done
dev1/bin/riak-admin cluster plan
dev1/bin/riak-admin cluster commit

for d in dev{5..6}; do $d/bin/riak-admin cluster join dev4@127.0.0.1; done
...
## kill some nodes
for d in dev{4..6}; do $d/bin/riak stop; done
...
watch -d ls dev{1..6}/data/bitcask
...
## force remove node 3
dev6/bin/riak-admin cluster force-remove dev3@127.0.0.1
