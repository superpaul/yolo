-module(frequency_dan).
-export([start/0, stop/0, allocate/0, deallocate/1, list/0]).
-export([init/0]).
...
%% Notes from testing:
%%   As seen in ni().
%%     frequency:start() results in 7 reductions
%%     frequency:allocate() results in 4 reductions per invocation
